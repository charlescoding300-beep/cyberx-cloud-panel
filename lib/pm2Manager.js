...
async function controlAllApps(userId, action) {
  const validActions = ['restart', 'stop', 'start'];
  if (!validActions.includes(action)) {
    return { ok: false, error: 'invalid action' };
  }
  const listResult = await listApps(userId);
  if (!listResult.ok) return listResult;
  if (!listResult.apps.length) return { ok: true, affected: 0, message: 'No apps to control' };

  const results = await Promise.all(
    listResult.apps.map((app) => controlApp(userId, app.name, action))
  );
  ...
  return {
    ...
    error: failed.length ? `${failed.length} of ${results.length} apps failed to ${action}` : null
  };
}

module.exports = { startApp, listApps, controlApp, controlAllApps, getLogs, namespacedName };
